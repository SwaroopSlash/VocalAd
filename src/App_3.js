...
const App = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      if (!u) {
        ...
        setUser(u);
        // If there was a pending download and user is now signed in, trigger it
  ...
  const signInWithGoogle = async () => {
    ...
    try {
      if (user && user.isAnonymous) {
        // Link the Google account to the existing anonymous account
        await linkWithPopup(user, provider);
      } else {
        // Fallback for normal sign in
  ...
  const handleEmailAuth = async (e) => {
    ...
    try {
      ...
      if (err.code === 'auth/email-already-in-use') setAuthError("Email is already registered. Please log in.");
      else if (err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') setAuthError("Incorrect email or password.");
  ...
  const handleDevReset = async () => {
    if (!user) return;
    const usageRef = doc(db, 'artifacts', appId, 'users', user.uid, 'usage', 'stats');
    await updateDoc(usageRef, { tier: 'free', videoCount: 0 });
  };
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
          <div className="bg-slate-800 rounded-[2.5rem] p-10 max-w-sm w-full shadow-2xl text-center space-y-6 border border-slate-700">
            {(modalReason === "signup" || modalReason === "login") ? (
              <>
                <div className="space-y-2">
                  ...
                  <p className="text-slate-400 text-sm">
                    {authMode === 'signup' ? "Sign up to save and download your HD commercial." : 
                     authMode === 'login' ? "Log in to access your projects and credits." : 
                     "Enter your email to receive a password reset link."}
                  </p>
                </div>

                {authError && <div className="p-3 bg-red-500/10 border border-red-500/20 text-red-400 text-xs rounded-xl font-bold text-left">{authError}</div>}
                {authMessage && <div className="p-3 bg-green-500/10 border border-green-500/20 text-green-400 text-xs rounded-xl font-bold text-left">{authMessage}</div>}

                <form onSubmit={handleEmailAuth} className="space-y-4">
                  <input type="email" placeholder="Email Address" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-4 bg-slate-900 border border-slate-700 rounded-2xl text-white text-sm focus:border-indigo-500 outline-none" required />
                  {authMode !== 'reset' && (
                    <input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full p-4 bg-slate-900 border border-slate-700 rounded-2xl text-white text-sm focus:border-indigo-500 outline-none" required />
                  )}
                  <button type="submit" className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-xl hover:bg-indigo-700 transition-all active:scale-95">
                    {authMode === 'signup' ? "Sign Up with Email" : authMode === 'login' ? "Sign In" : "Send Reset Link"}
                  </button>
                </form>

                {authMode === 'login' && (
                  <button onClick={() => { setAuthMode('reset'); setAuthError(''); setAuthMessage(''); }} className="text-indigo-400 text-xs font-bold hover:text-indigo-300">Forgot Password?</button>
                )}

                {authMode !== 'reset' && (
                  <>
                    <div className="flex items-center gap-3"><div className="h-px bg-slate-700 flex-1"></div><span className="text-xs text-slate-500 font-bold uppercase tracking-widest">OR</span><div className="h-px bg-slate-700 flex-1"></div></div>
                    ...
                    <button onClick={() => { setAuthMode(authMode === 'login' ? 'signup' : 'login'); setAuthError(''); setAuthMessage(''); }} className="text-slate-400 text-xs font-bold hover:text-white">
                ...
                {authMode === 'reset' && (
                  <button onClick={() => { setAuthMode('login'); setAuthError(''); setAuthMessage(''); }} className="text-slate-400 text-xs font-bold hover:text-white mt-4 block mx-auto">Back to Log In</button>
                )}
              </>
            ) : (
              <>
                <div className="w-20 h-20 bg-indigo-500/20 text-indigo-400 rounded-full flex items-center justify-center mx-auto ring-8 ring-indigo-500/5">
                  {modalReason === "limit" ? <Lock className="w-10 h-10" /> : <ShieldCheck className="w-10 h-10" />}
                </div>
                <div className="space-y-2">
                  <h3 className="text-2xl font-black text-white">{modalReason === "limit" ? "Limit Reached" : "Length Limit"}</h3>
                  <p className="text-slate-400 text-sm">
                    {modalReason === "limit" ? "You've used all credits. Upgrade for ₹50 to get 10 HD exports up to 2 minutes!" : "Free videos are capped at 30s. Upgrade to ₹50 for 2-minute ads!"}
                  </p>
                </div>
                {user && user.isAnonymous ? (
                  <button onClick={() => { setModalReason('signup'); setAuthMode('signup'); }} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-xl flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all active:scale-95">
                    Create Account to Continue
                  </button>
                ) : (
                  usage.tier === 'free' && (
                    <button onClick={handleUpgrade} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-xl flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all active:scale-95"><Zap className="w-5 h-5 fill-current" /> Buy 10 Credits (₹50)</button>
                  )
                )}
              </>
            )}
            <button onClick={() => { setShowAuthModal(false); setAuthError(''); setAuthMessage(''); }} className="text-slate-500 font-bold text-xs uppercase tracking-widest hover:text-slate-300 pt-2 block mx-auto">Close</button>
          </div>
        </div>
      )}
          ...
          <div className="absolute top-0 right-0 p-6 flex flex-col items-end gap-2">
            {user && !user.isAnonymous ? (
              <div className="flex items-center gap-2">
                 <button onClick={handleDevReset} className="text-[9px] font-bold text-amber-200 bg-amber-500/20 hover:bg-amber-500/30 px-3 py-1.5 rounded-full transition-all uppercase tracking-widest">
                   Reset Dev
                 </button>
                 <button onClick={handleSignOut} className="text-[10px] font-bold text-slate-300 bg-black/20 hover:bg-black/40 px-3 py-1.5 rounded-full transition-all">
                   Sign Out
                 </button>
                 <div className="text-[10px] font-bold text-white/80 flex items-center gap-1.5 bg-black/10 px-3 py-1.5 rounded-full shadow-inner">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => { setModalReason('login'); setAuthMode('login'); setShowAuthModal(true); }} className="text-[10px] font-bold text-white bg-indigo-500 hover:bg-indigo-400 px-4 py-1.5 rounded-full transition-all flex items-center gap-1.5 shadow-lg border border-indigo-400/50">
                  <User className="w-3 h-3" /> Sign In
                </button>
              </div>
            )}
            <div className="bg-black/20 backdrop-blur-xl px-5 py-2.5 rounded-full text-[10px] font-black tracking-widest border border-white/10 flex items-center gap-2 shadow-sm">
            ...
            <div className="py-16 space-y-12 animate-in zoom-in-95">
              ...
              <input id="imageInput" type="file" className="hidden" accept="image/*" onChange={(e) => {
                  ...
                  if (file) {
                    ...
                    reader.onload = (event) => { setImage(reader.result); setStep(1); };
                ...
                <div className="space-y-8">
                  <div className="grid grid-cols-2 gap-6 text-left">
                    <div className="space-y-2">
                      ...
                      <select className="w-full p-4 border-2 border-slate-700 rounded-2xl bg-slate-900 text-white font-bold text-sm focus:border-indigo-500 outline-none shadow-sm" value={selectedLanguage.code} onChange={(e) => { const lang = LANGUAGES.find(l => l.code === e.target.value); if (lang) setSelectedLanguage(lang); }}>
                        {LANGUAGES.map(l => <option key={l.code} value={l.code}>{l.label}</option>)}
                      </select>
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Speech Pace</label>
                      <select className="w-full p-4 border-2 border-slate-700 rounded-2xl bg-slate-900 text-white font-bold text-sm focus:border-indigo-500 outline-none shadow-sm" value={selectedSpeed.label} onChange={(e) => { const speed = SPEEDS.find(s => s.label === e.target.value); if (speed) setSelectedSpeed(speed); }}>
                ...
                {isGeneratingAudio && (
                  <div className="space-y-4 max-w-xl mx-auto w-full">
                    <div className="flex justify-between items-center text-[10px] font-black text-indigo-400 tracking-[0.2em] uppercase">
                      <span className="flex items-center gap-2 animate-pulse"><BrainCircuit className="w-3 h-3" /> {statusMessage}</span>
          ...
          {step === 4 && (
            <div className="text-center space-y-12 animate-in zoom-in-95 py-12">
              ...
              <div className="flex flex-col sm:flex-row gap-6 justify-center px-6 max-w-2xl mx-auto">
                <button onClick={() => {setImage(undefined); setStep(0); setText(""); setAudioUrl(undefined); setFinalVideoUrl(undefined);}} className="px-10 py-5 border-2 border-slate-700 rounded-[2rem] font-black uppercase text-xs hover:bg-slate-100 flex-1 transition-all text-slate-400 tracking-widest">New Project</button>
