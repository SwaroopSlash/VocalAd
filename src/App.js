...
const App = () => {
  ...
  const createVideo = async () => {
    ...
    try {
      ...
      // Apply transform (pan + zoom) for images
      if (assetType === 'image') {
                ...
                <div className="relative bg-black rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl border-4 md:border-8 border-slate-800 shrink-0" style={{ width: '200px', aspectRatio: '9/16' }}>
                  {assetType === 'image'
                    ? <img src={image} draggable={false}
                        ...
                        onPointerMove={(e) => { if (!panStartRef.current) return; const dx = e.clientX - panStartRef.current.mx; const dy = e.clientY - panStartRef.current.my; const maxX = Math.max(0, panStartRef.current.cw * (panStartRef.current.scale - 1) / 2); const maxY = Math.max(0, panStartRef.current.ch * (panStartRef.current.scale - 1) / 2); setImgTransform(prev => ({ ...prev, x: Math.max(-maxX, Math.min(maxX, panStartRef.current.tx + dx)), y: Math.max(-maxY, Math.min(maxY, panStartRef.current.ty + dy)) })); }}
